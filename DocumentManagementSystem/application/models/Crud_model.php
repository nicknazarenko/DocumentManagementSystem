...
class Crud_model extends CI_Model {
    ...
    function get_adminimage_url($type = '', $id = '') {
        if (file_exists('uploads/admin_image/' . $type . '_image/' . $id . '.jpg'))
            $image_url = base_url() . 'uploads/admin_image/' . $type . '_image/' . $id . '.jpg';
        else
    ...
    function save_document_repository_info_admin()
    {
        $data_repo['timestamp']         = strtotime($this->input->post('timestamp'));
        $data_repo['title']             = $this->input->post('title');
        $data_repo['description']       = $this->input->post('description');
        $data_repo['file_name']         = $_FILES["file_name"]["name"];
        $data_repo['file_type']         = $this->input->post('file_type');
        // $data_repo['class_id']          = $this->input->post('class_id');
        $data_repo['admin_id']        = $this->session->userdata('login_user_id');
        
        $this->db->insert('repository',$data_repo);
        
        $document_id            = $this->db->insert_id();
        move_uploaded_file($_FILES["file_name"]["tmp_name"], "uploads/repository/" . $_FILES["file_name"]["name"]);
    }
    
    function select_document_repository_info_admin()
    {
        $this->db->order_by("timestamp", "desc");
        return $this->db->where('admin_id', $this->session->userdata('login_user_id'))->get('repository')->result_array(); 
    }

    
    function update_document_repository_info_admin($document_id)
    {
        $repo_data['timestamp']      = strtotime($this->input->post('timestamp'));
        $repo_data['title']      = $this->input->post('title');
        $repo_data['description']    = $this->input->post('description');
        //$repo_data['class_id']   = $this->input->post('class_id');
        
        $this->db->where('document_id',$document_id);
        $this->db->update('repository',$repo_data);
    }
    
    function delete_document_repository_info_admin($document_id)
    {
        $this->db->where('document_id',$document_id);
        $this->db->delete('repository');
    }

        ////////TEACHER DOCUMENT REPOSITORY////////// 
    function save_document_repository_info()
    {
        $data_repo['timestamp']         = strtotime($this->input->post('timestamp'));
        $data_repo['title']             = $this->input->post('title');
        $data_repo['description']       = $this->input->post('description');
        ...
        $data_repo['file_type']         = $this->input->post('file_type');
        // $data_repo['class_id']          = $this->input->post('class_id');
        $data_repo['teacher_id']        = $this->session->userdata('login_user_id');
    ...
    function select_document_repository_info()
    {
        $this->db->order_by("timestamp", "desc");
        return $this->db->where('teacher_id', $this->session->userdata('login_user_id'))->get('repository')->result_array(); 
    }

    
    function update_document_repository_info($document_id)
    {
        $repo_data['timestamp']      = strtotime($this->input->post('timestamp'));
        $repo_data['title']      = $this->input->post('title');
        $repo_data['description']    = $this->input->post('description');
    ...
    function delete_document_repository_info($document_id)
    {
        $this->db->where('document_id',$document_id);
        $this->db->delete('repository');
    }

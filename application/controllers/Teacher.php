...
class Teacher extends CI_Controller
{
    ...
    function document_repository($task = "", $document_id = "")
    {
        ...
        if ($task == "create")
        {
            $this->crud_model->save_document_repository_info();
            $this->session->set_flashdata('flash_message' , get_phrase('repository_material_info_saved_successfuly'));
            redirect(base_url() . 'index.php?teacher/document_repository' , 'refresh');
        }
        
        
        if ($task == "update")
        {
            $this->crud_model->update_document_repository_info($document_id);
        ...
        if ($task == "delete")
        {
            $this->crud_model->delete_document_repository_info($document_id);
            redirect(base_url() . 'index.php?teacher/document_repository');
        }
        
        $data['document_repository_info']    = $this->crud_model->select_document_repository_info();
